...
module VotingRoomTests =
    ...
    [<Test>]
    let ``New VotingRoomAgent should have an empty Brainstorming state`` () =
        let agent = new VotingRoomAgent()
        retrieveState agent |> should equal (Brainstorming [])
    
    [<Test>]
    let ``Posting one AddOption message should add one option`` () =
        let agent = new VotingRoomAgent()
        agent.Post (AddOption "option a")
        retrieveState agent |> should equal (Brainstorming ["option a"])
    
    [<Test>]
    let ``Posting two AddOption messages should show two in reverse order`` () =
        let agent = new VotingRoomAgent()
        agent.Post (AddOption "option a")
        agent.Post (AddOption "option b")
        retrieveState agent |> should equal (Brainstorming ["option b"; "option a"])
